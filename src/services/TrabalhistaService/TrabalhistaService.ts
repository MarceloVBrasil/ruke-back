...
export class TrabalhistaService implements ITrabalhistaService {
    constructor(private trabalhistaRepository: ITrabalhistaRepository) { }
    ...
    async calcularTotalCausa(id: string): Promise<{ resultado: number }> {
        const ticket = await this.trabalhistaRepository.getById(id)
        const valor_total_causa =
            (ticket.reconhecimento_vinculo_empregaticio?.valor_estimado_pedido ?? 0)
    ...
    async add(tenant_id: string): Promise<Trabalhista> {
        const ticket: Trabalhista = {
